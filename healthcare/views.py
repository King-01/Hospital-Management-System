...
def search(request):
    ...
    if not request.GET.get('location', 'none') == 'none' and not request.GET.get('specialty', 'none') == 'none':
        ...
        hospitals = list()
        for obj in location:
            hospitalx = Hospital.objects.filter(location = obj)
            for x in hospitalx:
                hospitals.append(x)
        # hospitals = Hospital.objects.filter(location=location)
        doctors = []
        for hospital in hospitals:
            specialtyx = Department.objects.filter(department_name=specialty)
            for obj in specialtyx:
                doctorx = DoctorProfile.objects.filter(hospital=hospital, specialization=obj)
                for x in doctorx:
                    doctors.append(x)
...
class AppointmentUpdate(UpdateView):
    ...
    def form_valid(self, form):
        appointment = form.save(commit=False)
        if hasattr(self.request.user, 'userprofile') and appointment.cleaned_data['appointment_date'] > date.today():
