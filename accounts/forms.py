...
class UserForm(forms.ModelForm):
    password1 = forms.CharField(label='Password',widget=forms.PasswordInput(attrs={'placeholder':'Password','class':'form-control','id':'search-bar','required':''}))
    password2 = forms.CharField(label='Confirm_Password',widget=forms.PasswordInput(attrs={'placeholder':'Confirm_Password','class':'form-control','id':'search-bar','required':''}))
...
class ProfileForm(forms.ModelForm):
    ...
    birth_date = forms.DateField(label='Date_of_birth',widget=forms.TextInput(attrs={'type': 'date','placeholder':'Date Of Birth','class':'form-control','id':'search-bar'}))
    class Meta:
        model = UserProfile
...
class DoctorForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'specialization': forms.Select(attrs={'class': 'form-control','id':'search-bar','placeholder':'Specialization','required':''}),
        }
class UpdateUserProfile(forms.ModelForm):
    password1 = forms.CharField(label='Confirm_Password',widget=forms.PasswordInput(attrs={'placeholder':'Confirm_Password','class':'form-control','id':'search-bar','required':''}))
    password2 = forms.CharField(label='password',widget=forms.PasswordInput(attrs={'placeholder':'Password','class':'form-control','id':'search-bar','required':''}))
    #username = forms.CharField(label='username',widget=forms.TextInput(attrs={'placeholder':'Username','class':'form-control','id':'search-bar','required':''}))
    email = forms.EmailField(label='email',widget=forms.TextInput(attrs={'type':'email','placeholder':'Email','class':'form-control','id':'search-bar','required':''}))
    first_name = forms.CharField(label='first_name',widget=forms.TextInput(attrs={'placeholder':'First Name','class':'form-control','id':'search-bar','required':''}))
    last_name = forms.CharField(label='last_name',widget=forms.TextInput(attrs={'placeholder':'Last Name','class':'form-control','id':'search-bar','required':''}))
    class Meta:
        model = User
        fields = ['first_name', 'last_name','email', 'password2', 'password1']
